"""
===========================================
Celery 应用配置 (Celery Application)
===========================================

作用：
  配置和创建 Celery 应用实例

什么是 Celery？
  - 分布式任务队列
  - 用于处理异步任务和定时任务
  - 类似前端的 Web Worker，但在服务器端

为什么需要 Celery？
  1. 处理耗时任务（不阻塞 API 响应）
  2. 定时任务（定期清理、统计）
  3. 并发处理（批量任务）
  4. 任务重试（失败自动重试）

类比前端：
  - 类似 setTimeout/setInterval
  - 但更强大：分布式、可监控、自动重试

架构：
  FastAPI → 发送任务 → Redis (Broker) → Celery Worker → 执行任务 → Redis (Backend) → 存储结果
"""

from celery import Celery
from celery.schedules import crontab
from core.config import settings


# ============================================================
# 创建 Celery 应用实例
# ============================================================

celery_app = Celery(
    "fastapi_tasks",  # 应用名称
    broker=settings.CELERY_BROKER,  # 消息代理（Redis）
    backend=settings.CELERY_BACKEND,  # 结果存储（Redis）
    include=[
        "tasks.email_tasks",      # 邮件任务
        "tasks.report_tasks",     # 报表任务
        "tasks.cleanup_tasks",    # 清理任务
    ]
)
"""
Celery 应用实例

参数说明:
  - broker: 消息代理，存储待执行的任务
  - backend: 结果后端，存储任务执行结果
  - include: 自动导入的任务模块列表

为什么需要 Broker 和 Backend？
  - Broker: 存储任务队列（待办事项列表）
  - Backend: 存储任务结果（执行结果记录）

  就像：
    Broker = 待办事项 App
    Backend = 完成记录 App
"""


# ============================================================
# Celery 配置
# ============================================================

celery_app.conf.update(
    # =========================================
    # 任务配置
    # =========================================

    task_serializer="json",
    """
    任务序列化格式

    支持的格式: json, pickle, yaml, msgpack
    推荐 json（安全、兼容性好）
    """

    accept_content=["json"],
    """
    接受的内容类型

    安全起见，只接受 json
    """

    result_serializer="json",
    """结果序列化格式"""

    timezone="Asia/Shanghai",
    """
    时区设置

    影响定时任务的执行时间
    """

    enable_utc=False,
    """
    是否使用 UTC 时间

    False: 使用本地时区
    True: 使用 UTC（如果服务器在国外）
    """

    # =========================================
    # 任务执行配置
    # =========================================

    task_acks_late=True,
    """
    任务确认时机

    True: 任务执行完成后确认（推荐）
    False: 任务接收后立即确认

    好处：
    - 任务执行失败不会丢失
    - Worker 崩溃后任务可以重新执行
    """

    task_reject_on_worker_lost=True,
    """
    Worker 丢失时是否拒绝任务

    True: Worker 异常退出时，任务重新入队
    False: 任务丢失
    """

    worker_prefetch_multiplier=1,
    """
    Worker 预取任务数量

    1: 每次只取 1 个任务（公平分配）
    4: 每次取 4 个任务（提高吞吐量）

    建议：
    - 长任务: 1（避免阻塞）
    - 短任务: 4-8（提高效率）
    """

    task_time_limit=3600,
    """
    任务执行超时时间（秒）

    超过此时间，任务被强制终止
    3600 秒 = 1 小时

    防止任务卡死占用 Worker
    """

    task_soft_time_limit=3000,
    """
    任务软超时时间（秒）

    超过此时间，抛出 SoftTimeLimitExceeded 异常
    给任务一个清理的机会

    3000 秒 = 50 分钟
    """

    # =========================================
    # 结果配置
    # =========================================

    result_expires=3600,
    """
    任务结果过期时间（秒）

    结果在 Redis 中保留的时间
    3600 秒 = 1 小时

    过期后自动删除（节省内存）
    """

    result_backend_transport_options={
        "master_name": "mymaster",
    },
    """结果后端传输选项（Redis 哨兵模式用）"""

    # =========================================
    # 路由配置（高级）
    # =========================================

    task_routes={
        "tasks.email_tasks.*": {"queue": "email"},
        "tasks.report_tasks.*": {"queue": "report"},
        "tasks.cleanup_tasks.*": {"queue": "cleanup"},
    },
    """
    任务路由配置

    将不同类型的任务分配到不同的队列

    好处：
    - 优先级控制（重要任务优先）
    - 资源隔离（邮件任务不影响报表）
    - 独立扩展（邮件 Worker 独立扩展）

    使用：
    celery -A core.celery_app worker -Q email  # 只处理邮件队列
    celery -A core.celery_app worker -Q report # 只处理报表队列
    """

    # =========================================
    # 定时任务配置
    # =========================================

    beat_schedule={
        # 每天凌晨 2 点清理过期数据
        "cleanup-expired-data": {
            "task": "tasks.cleanup_tasks.cleanup_expired_data",
            "schedule": crontab(hour=2, minute=0),
        },

        # 每小时生成统计报表
        "generate-hourly-report": {
            "task": "tasks.report_tasks.generate_hourly_report",
            "schedule": crontab(minute=0),  # 每小时的 0 分
        },

        # 每 10 分钟检查一次
        "health-check": {
            "task": "tasks.cleanup_tasks.health_check",
            "schedule": 600.0,  # 600 秒 = 10 分钟
        },
    },
    """
    定时任务配置（Beat Schedule）

    定时任务类似 Linux 的 cron

    schedule 格式:
    - crontab(hour=2, minute=0)  # 每天 2:00
    - crontab(hour=0, minute=0, day_of_week=1)  # 每周一 0:00
    - crontab(minute="*/15")  # 每 15 分钟
    - 600.0  # 每 600 秒

    需要启动 Beat 进程：
    celery -A core.celery_app beat
    """
)


# ============================================================
# 任务基类配置
# ============================================================

class BaseTask(celery_app.Task):
    """
    自定义任务基类

    可以添加通用的任务行为

    示例：
    - 任务开始/结束时记录日志
    - 任务失败时发送通知
    - 统计任务执行时间
    """

    def on_success(self, retval, task_id, args, kwargs):
        """
        任务成功时调用

        参数:
            retval: 任务返回值
            task_id: 任务 ID
            args: 任务参数
            kwargs: 任务关键字参数
        """
        print(f"✅ 任务成功: {self.name} (ID: {task_id})")

    def on_failure(self, exc, task_id, args, kwargs, einfo):
        """
        任务失败时调用

        参数:
            exc: 异常对象
            task_id: 任务 ID
            args: 任务参数
            kwargs: 任务关键字参数
            einfo: 异常信息
        """
        print(f"❌ 任务失败: {self.name} (ID: {task_id})")
        print(f"   错误: {exc}")

    def on_retry(self, exc, task_id, args, kwargs, einfo):
        """
        任务重试时调用

        参数:
            exc: 异常对象
            task_id: 任务 ID
            args: 任务参数
            kwargs: 任务关键字参数
            einfo: 异常信息
        """
        print(f"🔄 任务重试: {self.name} (ID: {task_id})")


# 设置默认任务基类
celery_app.Task = BaseTask


# ============================================================
# 学习笔记
# ============================================================
"""
关键概念总结：

1. 【Celery 核心组件】
   - Celery App: 应用实例
   - Worker: 执行任务的进程
   - Beat: 定时任务调度器
   - Broker: 消息代理（存储任务队列）
   - Backend: 结果存储（存储任务结果）

2. 【任务队列流程】
   1. FastAPI 发送任务 → Broker (Redis)
   2. Worker 从 Broker 获取任务
   3. Worker 执行任务
   4. 结果存储到 Backend (Redis)
   5. FastAPI 从 Backend 获取结果

3. 【启动 Celery】
   # 启动 Worker（执行任务）
   celery -A core.celery_app worker --loglevel=info

   # 启动 Beat（定时任务调度）
   celery -A core.celery_app beat --loglevel=info

   # 启动 Flower（Web 监控）
   celery -A core.celery_app flower

4. 【队列 vs 普通函数】
   普通函数:
   - 同步执行，阻塞 API
   - 任务失败无法重试
   - 无法监控

   Celery 任务:
   - 异步执行，不阻塞 API
   - 自动重试
   - 可监控、可扩展

5. 【使用场景】
   - 发送邮件（耗时 1-2 秒）
   - 生成报表（耗时数分钟）
   - 图片处理（耗时数秒）
   - 数据导入/导出
   - 定时任务（每天统计）
"""
